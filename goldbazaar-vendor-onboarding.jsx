...
const VendorOnboardingPortal = () => {
  ...
  const validateStep = (stepIndex) => {
    ...
    if (stepIndex === 0) {
      ...
      } else if (isNaN(formData.yearsInBusiness) || formData.yearsInBusiness < 0 || formData.yearsInBusiness > 70) {
        newErrors.yearsInBusiness = 'Enter a valid number between 0 and 70';
      }
    } else if (stepIndex === 2) {
      // Address
      ...
      // Branding (optional but warn if empty)
      if (formData.services.length > 0) {
        // Just a reminder, not required
      }
    } else if (stepIndex === 5) {
      // Scheme Setup
      if (!formData.schemeName.trim()) {
        newErrors.schemeName = 'Scheme name is required';
      } else if (formData.schemeName.length < 3) {
        newErrors.schemeName = 'Scheme name must be at least 3 characters';
      }

      if (!formData.schemeDescription.trim()) {
        newErrors.schemeDescription = 'Scheme description is required';
      } else if (formData.schemeDescription.length < 10) {
        newErrors.schemeDescription = 'Description must be at least 10 characters';
      }

      if (!formData.benefits.trim()) {
        newErrors.benefits = 'Key benefits are required';
      } else if (formData.benefits.length < 5) {
        newErrors.benefits = 'Benefits must be at least 5 characters';
      }

      if (!formData.validity) {
        newErrors.validity = 'Validity period is required';
      } else if (isNaN(formData.validity) || formData.validity < 1 || formData.validity > 60) {
        newErrors.validity = 'Validity must be between 1 and 60 months';
      }
    }
          ...
          <div className="space-y-6">
            <div>
              ...
              <input
                ...
                } text-white rounded-lg focus:outline-none transition`}
              />
              {errors.mobile && (
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                } text-white rounded-lg focus:border-yellow-500 focus:outline-none transition`}
              />
              {errors.businessName && (
                <div className="flex items-center gap-2 mt-2 text-red-400 text-sm">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 bg-gray-800 border ${
                  errors.gstNumber ? 'border-red-500' : 'border-gray-700'
                } text-white rounded-lg focus:border-yellow-500 focus:outline-none transition`}
              />
              {errors.gstNumber && (
                <div className="flex items-center gap-2 mt-2 text-red-400 text-sm">
                  <AlertCircle className="w-4 h-4" /> {errors.gstNumber}
                </div>
              )}
              <p className="text-xs text-gray-400 mt-1">Format: 27ABCDE1234F1Z5</p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  max="70"
                  className={`w-full px-4 py-3 bg-gray-800 border ${
                    errors.yearsInBusiness ? 'border-red-500' : 'border-gray-700'
                  } text-white rounded-lg focus:border-yellow-500 focus:outline-none transition`}
                />
                {errors.yearsInBusiness && (
                  <div className="flex items-center gap-2 mt-2 text-red-400 text-sm">
                    <AlertCircle className="w-4 h-4" /> {errors.yearsInBusiness}
                  </div>
                )}
              </div>
            </div>
        ...
        return (
          <div className="space-y-4">
            ...
            <p className="text-xs text-gray-400 text-center mt-4">
              💡 Tip: High-quality images help attract more customers
            </p>
          </div>
        );
      case 5: // Scheme Setup
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 bg-gray-800 border ${
                  errors.schemeName ? 'border-red-500' : 'border-gray-700'
                } text-white rounded-lg focus:border-yellow-500 focus:outline-none transition`}
              />
              {errors.schemeName && (
                <div className="flex items-center gap-2 mt-2 text-red-400 text-sm">
                  <AlertCircle className="w-4 h-4" /> {errors.schemeName}
                </div>
              )}
            </div>
            <div>
              ...
              <textarea
                ...
                rows="3"
                className={`w-full px-4 py-3 bg-gray-800 border ${
                  errors.schemeDescription ? 'border-red-500' : 'border-gray-700'
                } text-white rounded-lg focus:border-yellow-500 focus:outline-none transition`}
              />
              {errors.schemeDescription && (
                <div className="flex items-center gap-2 mt-2 text-red-400 text-sm">
                  <AlertCircle className="w-4 h-4" /> {errors.schemeDescription}
                </div>
              )}
            ...
            <div>
              ...
              <input
                ...
                onChange={handleInputChange}
                placeholder="e.g., Low interest, Fast approval, Flexible tenure"
                className={`w-full px-4 py-3 bg-gray-800 border ${
                  errors.benefits ? 'border-red-500' : 'border-gray-700'
                } text-white rounded-lg focus:border-yellow-500 focus:outline-none transition`}
              />
              {errors.benefits && (
                <div className="flex items-center gap-2 mt-2 text-red-400 text-sm">
                  <AlertCircle className="w-4 h-4" /> {errors.benefits}
                </div>
              )}
            </div>
            <div>
              ...
              <input
                ...
                max="60"
                className={`w-full px-4 py-3 bg-gray-800 border ${
                  errors.validity ? 'border-red-500' : 'border-gray-700'
                } text-white rounded-lg focus:border-yellow-500 focus:outline-none transition`}
              />
              {errors.validity && (
                <div className="flex items-center gap-2 mt-2 text-red-400 text-sm">
                  <AlertCircle className="w-4 h-4" /> {errors.validity}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-800 rounded-lg p-4 space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-white font-medium">{formData.schemeName || 'Not provided'}</span>
              </div>
            </div>
            <p className="text-xs text-gray-400 text-center mt-4">
